...
func processMapTask(task *TaskDesc, mapf func(string, string) []KeyValue) {
	fmt.Printf("Map Task %d started on worker %v\n", task.TaskID, myId())
	// Map Task
...
func processReduceTask(task *TaskDesc, reducef func(string, []string) string) {
	fmt.Printf("Reduce Task %d started on worker %v\n", task.TaskID, myId())
	// Reduce Task
...
func Worker(mapf func(string, string) []KeyValue,
	...
	for {
		...
		case 0:
			...
			CallReportTaskCompletion(0, task.TaskID)
		case 1:
...
func CallReportTaskCompletion(taskType int, taskID int) bool {
	args := TaskDesc{
		TaskType: taskType,
		TaskID:   taskID,
	}
